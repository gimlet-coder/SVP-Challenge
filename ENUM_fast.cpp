...
bool ENUM_fast(const RealMatrix &U, const RealVector &B_norm, const RealVector &R_squares, IntVector &v_out, const int k_begin, const int k_end, long long &node_count){
    ...
    Eigen::Matrix<FastScalar, Eigen::Dynamic, Eigen::Dynamic> Sigma(n + 2, n + 2); 
    Sigma.setZero();
    Eigen::Matrix<FastScalar, Eigen::Dynamic, 1> rho(n + 2);
    rho.setZero();
    ...
    v_temp.setZero();
    Eigen::Matrix<FastScalar, Eigen::Dynamic, 1> c(n);
    c.setZero();
    ...
    while(true){ // step.9
        ...
        FastScalar diff = static_cast<FastScalar>(v_temp(k - 1)) - c(k - 1);

        rho(k) = rho(k + 1) + diff * diff * B_norm_fast(global_k); // step.10
...
std::cerr << "\n見つかったノルム: " << R_squares(k - 1) << " (Nodes: " << node_count << ")" << std::endl;
            ...
            for (int i_local = r[k]; i_local >= k + 1; i_local--) {
                ...
                int g_k = k + k_begin - 1;
                Sigma(i_local, k - 1) = Sigma(i_local + 1, k - 1) + static_cast<FastScalar>(v_temp(i_local - 1)) * U_fast(g_i, g_k);
            }
            // step.20-23: c_k, v_k, w_k の設定
            c(k - 1) = -Sigma(k + 1, k - 1);
            FastScalar rounded_c_k = std::round(c(k - 1));
