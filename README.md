...
### 3. アルゴリズムにおける改良の余地について:
* 現状のプログラムでレコード記録の最短ベクトルを求めることに成功していることから、より実装コストを下げて高速化させていく方針を取りたい。
    * そのために、単純計算をGPUに演算させる方針や BKZ 内部の GSO 更新を Gram_Schmidt 関数を呼び出さずに GSOUpdate-BKZ を作成して計算オーダーを下げる方針が考えられる。
