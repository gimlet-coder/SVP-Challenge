...
### 2.2 Progressive BKZ による 80次元 SVP探索達成

| パラメータ | 設定値 |
| :--- | :--- |
| **次元 $n$** | 80 |
| **ブロックサイズ $\beta$** | 20 → 60 (少しずつ増加させる Progressive方式) |
| **見つかった最短ベクトルノルム** | **2322.92** |
| **総実行時間** | 63433.00秒 (約17時間37分) |

* **戦略**: `fplll` により `Progressive_BKZ` にて実行する最初のブロックサイズで BKZ を行い、格子をある程度ならしてから自作の `DeepBKZ` に渡して計算時間を短縮させた。
* **成果**: 一般家庭用のPCを用いて80次元の最短ベクトル探索を終了できた。しかしながら、現状の80次元の記録の最下位記録にすら及ばない結果なので改善の余地がある。

---

### 2.3 アルゴリズムにおける改良ポイント

* **LLLとDeepLLLのハイブリッド導入**:
    初期計算のボトルネック解消を目指し、 **LLL ($\delta = 0.75$)** による高速な整地と **DeepLLL ($\delta = 0.999$)** による高精度な簡約のハイブリッド構造を採用。これにより、初期簡約の品質を保ちながら高速化に成功した。なお、このステップは `fplll` を実行できていれば不要なので Windows 環境のみで行うようにしている。

* **$\beta$ 増分ロジック**:
    計算コストが増大することが予想される $\beta \ge 25$ 以降は、増分を $+3$ や $+5$ と調整した。これにより、計算時間削減と最短ベクトル発見のバランスを図った。

* **fplllを用いた格子の軽量化**:
    初期状態の格子は第一列の成分が異常に大きいため、`fplll` の BKZ を一度実行することにより格子を軽量化させた。ただし、Windows 環境で `fplll` を導入することはできなかったため WSL を用いて実行した。
    > **Note:** このリポジトリはあくまで Windows 環境での実行を前提としているので、より高次元の格子に実行したい場合は WSL または各自 Linux の環境で Eigen, Boost, fplll を導入したうえで実行してほしい。
...
* **ライブラリ:** **Eigen** (行列演算), **Boost** (多倍長演算), **fplll** (初期格子の軽量化 `Linux` のみ)
...
### Upgrade
* **多倍長浮動小数と多倍長整数のハイブリッド構造の実装**
    * 浮動小数の丸め誤差を回避するために、整数演算で行うべきところを多倍長整数を用いて修正しました。
* **ENUMの探索アルゴリズムに線形な枝刈りを追加**
* **Option: fplllに対応 (Linux環境のみ)**
    * Linux環境で `fplll` を導入すると、より高次元の格子の最短ベクトル探索をしやすくなります。

---

## 6. 実行方法

本プロジェクトは **MSYS2 UCRT64** 環境でのビルドを前提としています。WSL 等の Linux 環境は各自で導入してください。
