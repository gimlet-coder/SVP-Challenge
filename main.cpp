...
int main() {
    ...
    int min_val = 1'000'000, max_val = 100'000'000; // 乱数範囲
    ...
    try{
        ...
        auto end_time = std::chrono::high_resolution_clock::now(); // 終了時間を保存
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time); // 差分で所要時間を求める
...
// 出力(B_long_double)の最初の行の長さ
    std::cout << "DeepLLL後: " << B_long_double.row(0).squaredNorm() << std::endl;
    return 0;
}
